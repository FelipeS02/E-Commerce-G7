export function validate(input) {
  ...
  if (!input.name) {
    ...
    errors.name =
      "Product-name is invalid, the name must be at least four characters long and must be in uppercase letters";
  }
  if (!input.price) {
    ...
    !(
      ...
      /^([0]||[1-9][0-9][0-9]|[1-9][0-9][0-9][0-9]|[1-2][0-9][0-9][0-9][0-9])$/.test(
        input.price
      )
    )
  ) {
    errors.price =
      "Price is invalid, the price must be in a range of 100 to 29999 or it can be 0";
  }
  if (!input.color) {
    ...
    ![
      ...
      "tan",
      "granate",
    ].includes(input.color.toLowerCase())
  ) {
  ...
  if (!input.detail) {
    ...
    errors.detail =
      "Detail is invalid, the detail must start with a capital letter and have a minimum length of 50 characters";
  }
  if (!input.type) {
    errors.type = "Type is required";
  } else if (!(input.type.length > 3)) {
    errors.type =
      "Type is invalid, the type have a minimum length of 3 characters";
  }
  ...
  if (input.sizeStock.length > 0) {
    for (let i = 0; i < input.sizeStock.length; i++) {
      ...
      if (input.sizeStock[i].stock < 1) {
        errors[`stock${i}`] =
          "The stock is invalid, the stock must be in a range from 0 to 999";
      }
    }
  }
  if (!input.categories) {
    ...
    errors.categories =
      "Category is invalid, select at least one category or define a new category";
  }
  ...
  if (input.newCategories.length > 0) {
    for (let i = 0; i < input.newCategories.length; i++) {
      if (input.newCategories[i].name === "") {
        ...
        errors[`newCategory${i}`] =
          "New category is invalid, the new category have a minimum length of 3 characters";
      }
    }
  }
